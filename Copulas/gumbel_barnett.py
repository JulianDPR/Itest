...
def Gumbel_Barnett(XY, alpha, diff = False):
...
def inv_Gumbel_Barnett(u1, c, alpha):
    ...
    for i,j in zip(y,c):
        ...
        x.append(so.root_scalar(f, args=(alpha, i, j), bracket = (0, 20), method="brenth" ,xtol=1e-6).root)
     
       # except:
            
        #    x.append(so.root_scalar(f, args=(alpha, i, j), bracket = (-20, 0), method = "brentq").root)
